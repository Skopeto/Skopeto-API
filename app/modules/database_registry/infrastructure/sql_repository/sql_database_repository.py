...
from app.modules.database_registry.infrastructure.sql_query.sql_database_query import create_database_health_query, create_database_query, get_database_by_name_query, get_database_health_query, get_databases_query, update_database_health_query
...
class SqlDatabaseRepository(DatabaseRepositoryInterface, SqlBaseRepository):
    async def get_databases_by_server_id(self, server_id: int) -> list[Database] | None:
        try:
            ...
            logger.error(f"Database error while fetching databse with server ID {server_id}: {str(e)}", exc_info=True)
    ...
    async def persist_database(self, database: Database) -> Database:
        try:
            ...
            logger.error(f"Database error while persisting database {database.name}: {str(e)}", exc_info=True)
    ...
    async def get_database_by_name(self, name: str) -> Database | None:
        try:
            ...
            logger.error(f"Database error while fetching database with name {name}: {str(e)}", exc_info=True)
    ...
    async def persist_database_health(self, database_health: DatabaseHealth) -> DatabaseHealth:
        try:
            ...
            logger.error(f"Database error while saving database health for database {database_health.database_id}: {str(e)}", exc_info=True)
