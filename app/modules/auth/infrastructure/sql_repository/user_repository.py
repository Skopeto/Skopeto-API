...
class SqlUserRepository(UserRepositoryInterface, SqlBaseRepository):
    ...
    async def update_user(self, user: User) -> User:
        try:
            query, params = update_user_query(user)
            sql_query = SqlQuery(self.session, query, params)
            await sql_query.persist()
            return user
        except Exception as e:
            logger.error(f"Database error while updating user {user.id}: {str(e)}", exc_info=True)
            raise RepositoryException("Failed to update user")